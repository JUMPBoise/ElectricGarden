// theramin_build2.ino by C.Spindler 1-25-2020

// worked operationally at 1:00 pm on 1-26-2020
// freeze in this version and goto theramin_build3.ino
// for refinements and problem fixes:
//      1. tone are coming in notes about one per second, too slow
//      2. pitch range could be better
...
#define TOPRANGE 1000
#define LOSTEP 0
#define HISTEP 128

int rangeStep = (TOPRANGE-BOTTOMRANGE)/(HISTEP - LOSTEP);
    int volumeLevel = LOSTEP;
    if (dist >= TOPRANGE ){
      volumeLevel = HISTEP;
      } else if (dist >= (BOTTOMRANGE + rangeStep + 1)){
          // careful, intermediate product may be 32 bits long
          volumeLevel = (int)((dist-BOTTOMRANGE)*HISTEP/(TOPRANGE-BOTTOMRANGE));
        }
